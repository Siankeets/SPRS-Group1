...
function handleMenu(key){
  if(key === 'scan' || key === 'scanAttendance'){ 
    scanModal.style.display = 'flex';
    scanResult.textContent = '';

    const apiUrl = key === 'scan' 
                   ? "../admin/verify_qr-test.php" 
                   : "../admin/mark_attendance-test.php";
    ...
    qrScanner = new QrScanner(qrVideo, result => {
      ...
      .catch(err => {
          console.error(err);
          scanResult.textContent = "Server error while processing QR.";
      });
    });
      ...
      qrScanner.start();
      break;
    case 'redeem': window.location.href = 'student/redeem-test.php'; break; //changed to test ver. + root relative path (try absolute path as well)
