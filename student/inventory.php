      ...
      <section class="content">
        ...
        <div class="redeem-section" id="inventoryFlow">
          ...
          <ul class="option-list">
            <li class="option-item" onclick="showCategory('Point Vouchers')">Point Vouchers</li>
            <li class="option-item" onclick="showCategory('Tickets')">Tickets</li>
            <li class="option-item" onclick="showCategory('Event Certificates')">Event Certificates</li>
            <li class="option-item" onclick="showCategory('History')">History</li>
          </ul>
        </div>
      </section>
    </div>
...
<script>
const flow = document.getElementById('inventoryFlow');
let credits = 120; // initial credits

function showCategory(category) {
  if (category === 'History') {
    flow.innerHTML = `
      <h3>${category}</h3>
      <p>Displaying your previous redemptions and inventory updates.</p>
  ...
  flow.innerHTML = `
    <h3>${category}</h3>
    <p>These are the available items under <strong>${category}</strong>.</p>
    <ul class="option-list">
      <li class="option-item" onclick="redeemItem('${category} Item 1', 50)">Redeem ${category} Item 1 (50 pts)</li>
      <li class="option-item" onclick="redeemItem('${category} Item 2', 75)">Redeem ${category} Item 2 (75 pts)</li>
    </ul>
    <button onclick="resetFlow()">↩ Back to Inventory</button>
  `;
}
...
function redeemItem(item, cost) {
  flow.innerHTML = `
    <h3>Redeem ${item}</h3>
    <p>This reward costs <strong>${cost} points</strong>.</p>
    <button onclick="confirmRedeem('${item}', ${cost})">✅ Redeem</button>
    <button onclick="resetFlow()">❌ Cancel</button>
  `;
}

function confirmRedeem(item, cost) {
  if (credits >= cost) {
    credits -= cost;
    document.getElementById('credits').textContent = credits;
    flow.innerHTML = `
      <h3 style="color:var(--success)">🎉 Success!</h3>
      <p>You have redeemed <strong>${item}</strong> for ${cost} points.</p>
      <button onclick="resetFlow()">↩ Back to Inventory</button>
    `;
  } else {
    flow.innerHTML = `
      <h3 style="color:#ef4444">❌ Not Enough Points</h3>
      <p>You need ${cost - credits} more points to redeem this item.</p>
      <button onclick="resetFlow()">↩ Back</button>
    `;
  }
}
